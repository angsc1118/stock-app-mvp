...
# 
# 修改歷程:
# 2026-07-13 10:00:00: [Feat] 新增 generate_alerts()：集中化盤中告警邏輯 (停損/現金水位/突破/跌破/爆量/乖離)，供首頁與盤中監控頁共用
...
# --- [新增] 盤中告警集中化邏輯 (供首頁與盤中監控頁共用) ---
def generate_alerts(df_unrealized, cash_ratio, quotes, watchlist_df, ta_data, mp_table, current_time_str,
                     stop_loss_threshold=-20, cash_critical=10, cash_high=80,
                     volume_spike_ratio=2.0, bias_threshold=20):
    """
    彙整全部告警，回傳統一格式清單。純函式，零 Streamlit 依賴。

    輸入：
      df_unrealized : calculate_unrealized_pnl() 的輸出 (需含 股票代號/股票名稱/報酬率 (%))
      cash_ratio    : float，現金佔總資產比例 (%)
      quotes        : {股票代號: {price, change_pct, volume, last_updated}} (get_batch_detailed_quotes 輸出)
      watchlist_df  : database.load_watchlist() 輸出 (需含 股票代號/股票名稱/警示價_高/警示價_低)
      ta_data       : {股票代號: {Signal, MA20, Bias, Vol10}}
      mp_table      : database.load_mp_table() 輸出
      current_time_str : "HH:MM"，用於計算量能倍數

    輸出：list[dict]，每筆：
      { "symbol": str, "name": str, "type": str, "severity": "critical"|"warn"|"info",
        "message": str, "icon": str }
    """
    alerts = []

    # --- 1. 現金水位 (單一告警，非個股) ---
    if cash_ratio < cash_critical:
        alerts.append({
            "symbol": "", "name": "現金水位", "type": "cash_level", "severity": "critical",
            "message": f"現金比例過低 ({cash_ratio:.1f}%)，低於 {cash_critical}%", "icon": "🔴"
        })
    elif cash_ratio > cash_high:
        alerts.append({
            "symbol": "", "name": "現金水位", "type": "cash_level", "severity": "warn",
            "message": f"現金比例偏高 ({cash_ratio:.1f}%)，高於 {cash_high}%", "icon": "🟡"
        })

    # --- 2. 停損 (逐檔庫存，依未實現損益報表) ---
    if df_unrealized is not None and not df_unrealized.empty:
        danger = df_unrealized[df_unrealized['報酬率 (%)'] < stop_loss_threshold]
        for _, row in danger.iterrows():
            alerts.append({
                "symbol": row['股票代號'], "name": row['股票名稱'], "type": "stop_loss", "severity": "critical",
                "message": f"未實現報酬率 {row['報酬率 (%)']:.1f}%，已跌破停損容忍帶 ({stop_loss_threshold}%)", "icon": "🔴"
            })

        # --- 3. 最佳表現 (資訊性，非告警但沿用既有顯示習慣) ---
        best_stock = df_unrealized.sort_values('報酬率 (%)', ascending=False).iloc[0]
        if best_stock['報酬率 (%)'] > 0:
            alerts.append({
                "symbol": best_stock['股票代號'], "name": best_stock['股票名稱'], "type": "best_performer", "severity": "info",
                "message": f"目前庫存表現最佳，報酬率 +{best_stock['報酬率 (%)']:.1f}%", "icon": "🏆"
            })

    # --- 4. 突破 / 跌破 / 爆量 / 乖離 (逐檔，依 quotes + watchlist + ta_data) ---
    multiplier = get_volume_multiplier(current_time_str, mp_table)
    watchlist_df = watchlist_df if watchlist_df is not None else pd.DataFrame(columns=['股票代號', '股票名稱', '警示價_高', '警示價_低'])

    for symbol, quote in (quotes or {}).items():
        price = quote.get('price', 0)
        vol = quote.get('volume', 0)

        watch_info = watchlist_df[watchlist_df['股票代號'] == symbol]
        name = ""
        high_limit = 0
        low_limit = 0
        ...
        ta = (ta_data or {}).get(symbol, {})
        bias = ta.get('Bias', 0)
        vol_10ma = ta.get('Vol10', 0)
        _, vol_ratio = calculate_volume_ratio(vol, vol_10ma, multiplier)

        if high_limit > 0 and price >= high_limit:
            alerts.append({
                "symbol": symbol, "name": name, "type": "breakout", "severity": "info",
                "message": f"突破目標價 {high_limit} (現價 {price})", "icon": "🔔"
            })
        if low_limit > 0 and price > 0 and price <= low_limit:
            alerts.append({
                "symbol": symbol, "name": name, "type": "breakdown", "severity": "critical",
                "message": f"跌破支撐價 {low_limit} (現價 {price})", "icon": "💔"
            })
        if vol_ratio > volume_spike_ratio:
            alerts.append({
                "symbol": symbol, "name": name, "type": "volume_spike", "severity": "warn",
                "message": f"爆量，量比 {vol_ratio:.2f}", "icon": "🔥"
            })
        if bias > bias_threshold:
            alerts.append({
                "symbol": symbol, "name": name, "type": "bias", "severity": "warn",
                "message": f"月線乖離過大 (BIAS {bias:.2f}%)", "icon": "⚠️"
            })
