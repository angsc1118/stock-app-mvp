...
# 
# 修改歷程:
# 2025-11-24: [UI] 將戰情室控制台從 Sidebar 移至主畫面頂部，方便快速操作
...
with st.container(border=True):
    ...
    with col_ctrl_2:
        if not df_raw.empty:
            ...
            st.info("無交易資料，無法記錄資產")

# 提示訊息移至下方或保留
if df_raw.empty:
    st.info("💡 提示：目前沒有任何交易資料，請前往左側「帳務管理」頁面新增第一筆交易。")
...
# 4. 主程式執行
if not df_raw.empty:
    # 保留原本的盤中自動更新開關，但因為控制台已移出 Sidebar，我們可以把這個開關放在頁面右上角或控制台下方
    # 這裡將其簡化，若您希望保留開關，可以加在控制台區塊內
    # col_toggle, _ = st.columns([2, 8]) 
    # auto_refresh_on = col_toggle.toggle("啟用盤中自動更新 (每60秒)", value=False)
    
    # 預設傳入 False，若需自動更新功能可在此擴充
    render_dashboard(df_raw, auto_refresh=False)